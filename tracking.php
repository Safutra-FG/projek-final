...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['konfirmasi_servis'])) {
    ...
    try {
        if ($aksi == 'setuju') {
            $sql_update = "UPDATE service SET customer_approved = 1 WHERE id_service = ?";
            ...
            $new_status = 'Dibatalkan oleh Customer';
            $sql_update = "UPDATE service SET status = ? WHERE id_service = ?";
            $stmt_update = $koneksi->prepare($sql_update);
            if (!$stmt_update) throw new Exception("Prepare statement gagal.");
...
if ($id_service_input) {
    if (empty($id_service_input)) {
        ...
        $sql = "SELECT
                    s.id_service, s.tanggal, s.device, s.keluhan, s.status, s.customer_approved,
        ...
        if ($stmt) {
            ...
            if ($hasil->num_rows > 0) {
                ...
                $service_info = [
                    'id_service' => $row['id_service'], 'tanggal' => $row['tanggal'], 'nama_customer' => $row['nama_customer'],
                    'device' => $row['device'], 'keluhan' => $row['keluhan'], 'status' => $row['status'],
                    'customer_approved' => $row['customer_approved'], 'estimasi_waktu' => $row['estimasi_waktu'],
                    'estimasi_harga' => $row['estimasi_harga'], 'tanggal_selesai' => $row['tanggal_selesai'],
                ];
                ...
                while ($row_transaksi = $hasil_transaksi->fetch_assoc()) {
                    ...
                    if ($first_row) { $status_pembayaran = $row_transaksi['status']; $first_row = false; }
                }
                ...
                if (!empty($list_id_transaksi)) {
                    ...
                    if($stmt_total_bayar) {
                        ...
                        if ($result_total && $row_total = $result_total->fetch_assoc()) { $total_bayar = $row_total['total'] ?: 0; }
...
<div class="main-content">
    <div class="main-header"><h2 class="h4 text-dark mb-0 text-center flex-grow-1">Tracking Service</h2></div>
    ...
    <div class="flex-grow-1 p-3">
        ...
        <?php if ($service_info): ?>
            <div class="service-details">
                ...
                <div class="detail-row"><div class="detail-label">Tanggal Masuk:</div><div class="detail-value"><?php echo htmlspecialchars($service_info['tanggal']); ?></div></div>
                <div class="detail-row"><div class="detail-label">Nama Customer:</div><div class="detail-value"><?php echo htmlspecialchars($service_info['nama_customer']); ?></div></div>
                <div class="detail-row"><div class="detail-label">Perangkat:</div><div class="detail-value"><?php echo htmlspecialchars($service_info['device']); ?></div></div>
                <div class="detail-row"><div class="detail-label">Keluhan Utama:</div><div class="detail-value"><?php echo nl2br(htmlspecialchars($service_info['keluhan'])); ?></div></div>
                <div class="status-box" style="padding: 10px; margin-top: 15px; margin-bottom: 15px;"><div class="status-title" style="font-size: 1rem;">Status Service</div><div class="status-value" style="font-size: 1.2rem;"><?php echo htmlspecialchars($service_info['status']); ?></div></div>

                <?php
                if (
                    isset($service_info['status']) &&
                    strtolower($service_info['status']) == 'dikonfirmasi' &&
                    $service_info['customer_approved'] == 0 && 
                    !empty($service_info['estimasi_harga']) && !empty($service_info['estimasi_waktu'])
                ) :
                ?>
                <div class="card mt-4 mb-4 border-warning shadow">
                    <div class="card-header bg-warning text-dark"><strong><i class="fas fa-exclamation-triangle"></i> Mohon Konfirmasi Anda</strong></div>
                    <div class="card-body">
                        <p>Estimasi perbaikan untuk perangkat Anda telah kami siapkan. Mohon berikan konfirmasi apakah Anda setuju untuk melanjutkan perbaikan.</p>
                        <div class="detail-row"><div class="detail-label">Estimasi Biaya:</div><div class="detail-value fw-bold fs-5 text-danger">Rp <?php echo number_format($service_info['estimasi_harga'], 0, ',', '.'); ?></div></div>
                        <div class="detail-row"><div class="detail-label">Estimasi Waktu Pengerjaan:</div><div class="detail-value fw-bold"><?php echo htmlspecialchars($service_info['estimasi_waktu']); ?></div></div>
                        <p class="mt-3 small text-muted">Catatan: Biaya dan waktu bersifat estimasi.</p>
                        <hr>
                        <form method="POST" action="tracking.php">
                            ...
                            <p><strong>Apakah Anda ingin melanjutkan proses service?</strong></p>
                            <button type="submit" name="aksi" value="setuju" class="btn btn-success me-2"><i class="fas fa-check"></i> Ya, Lanjutkan Service</button>
                            <button type="submit" name="aksi" value="batal" class="btn btn-danger" onclick="return confirm('Apakah Anda yakin ingin membatalkan service ini?');"><i class="fas fa-times"></i> Tidak, Batalkan Service</button>
                        </form>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="detail-row"><div class="detail-label">Estimasi Waktu:</div><div class="detail-value"><?php echo htmlspecialchars($service_info['estimasi_waktu'] ?: '-'); ?></div></div>
                <div class="detail-row"><div class="detail-label">Estimasi Biaya Awal:</div><div class="detail-value">Rp <?php echo number_format($service_info['estimasi_harga'] ?: 0, 0, ',', '.'); ?> (Ini hanya perkiraan awal)</div></div>
                <div class="detail-row"><div class="detail-label">Tanggal Selesai:</div><div class="detail-value"><?php echo htmlspecialchars($service_info['tanggal_selesai'] ?: '-'); ?></div></div>
                ...
                <?php if (!empty($service_details_list)): ?>
                    ...
                    <?php foreach ($service_details_list as $index => $detail): ?>
                        <div class="detail-item-box">
                            ...
                            <?php if (!empty($detail['nama_barang'])): ?><div class="detail-row"><div class="detail-label">Sparepart:</div><div class="detail-value"><?php echo htmlspecialchars($detail['nama_barang']); ?></div></div><?php endif; ?>
                            <?php if (!empty($detail['jenis_jasa'])): ?><div class="detail-row"><div class="detail-label">Jasa Tambahan:</div><div class="detail-value"><?php echo htmlspecialchars($detail['jenis_jasa']); ?></div></div><?php endif; ?>
                            <div class="detail-row"><div class="detail-label">Deskripsi Pengerjaan:</div><div class="detail-value"><?php echo nl2br(htmlspecialchars($detail['detail_kerusakan_deskripsi'])); ?></div></div>
                            <div class="detail-row"><div class="detail-label">Biaya Item Ini:</div><div class="detail-value">Rp <?php echo number_format($detail['detail_total'], 0, ',', '.'); ?></div></div>
                        </div>
                    <?php endforeach; ?>
                    <div class="total-aktual-box"><div class="detail-row"><div class="detail-label">TOTAL TAGIHAN:</div><div class="detail-value">Rp <?php echo number_format($total_biaya_aktual_dari_detail, 0, ',', '.'); ?></div></div></div>
                ...
                if ($service_info && $jumlah_final_untuk_dibayar > 0 &&
                    (
                        ...
                        (strtolower($service_info['status']) == 'dikonfirmasi' && $service_info['customer_approved'] == 1)
                    ) &&
